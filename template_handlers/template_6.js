...
// Template 6-specific price
const PRICE = 10000;

// Template 6-specific background images
...
async function handle(ctx, { User, geminiModel, showLoading, logger, bot, fs, onComplete }) {
  ...
  // Gemini’dan 11 ta reja so‘rash
  const outlinePrompt = `${presentationData.topic} mavzusi uchun 
"birinchireja$ikkinchireja$...$o'ninchireja&o'nbirinchi" formatida faqat 11 ta reja yoz. 
Ortiqcha matn va raqam qo‘shma! // Misol: 
"Kirish$Tikuvchilik tarixi$Tikuvchilik turlari$Matolar va xususiyatlari$Tikuvchilik asbob-uskunalari$Andaza va bichish jarayoni$Tikish texnikalari$Zamonaviy tikuvchilik tendensiyalari$Tikuvchilikning iqtisodiy ahamiyati$Tikuvchilikning ijtimoiy ahamiyati$Xulosa"`;
  ...
  const page4Result = await geminiModel.generateContent(
    `${presentationData.topic} mavzusining ${plan[1]} rejasi asosida 100 - 120 so'zlardan iborat 1 paragraf ma'lumot ber!`
  );
  ...
  const page5Result = await geminiModel.generateContent(
    `${presentationData.topic} mavzusining ${plan[2]} rejasi asosida 100 - 120 so'zlardan iborat 1 paragraf ma'lumot ber!`
  );
  ...
  const page6Result = await geminiModel.generateContent(
    `${presentationData.topic} mavzusining ${plan[3]} rejasi asosida har biri 30 ta so'zdan iborat bo'lgan 3 ta paragraf ma'lumot ber. Javob "paragraf1$$$paragraf2$$$paragraf3" ko'rinishida har bir paragraf $$$ bilan ajratilgan bo'lsin. Har bor paragrafda 30ta so'zdan iborat bo'lsin`
  );
  ...
  const page7Result = await geminiModel.generateContent(
    `${presentationData.topic} mavzusining ${plan[4]} rejasi asosida 80 - 90 so'zlardan iborat 1 paragraf ma'lumot ber!`
  );
  ...
  eightSlide.addText(plan[5].toUpperCase(), {
    x: 0.5,
    y: 0.6,
    align: "center",
    fontSize: 22,
  ...
  const page9Result = await geminiModel.generateContent(
    `${presentationData.topic} mavzusining ${plan[6]} rejasi asosida 80 - 90 so'zlardan iborat 1 paragraf ma'lumot ber!`
  );
  ...
  const page10Result = await geminiModel.generateContent(
    `${presentationData.topic} mavzusining ${plan[7]} rejasi asosida 80 - 90 so'zlardan iborat 1 paragraf ma'lumot ber!`
  );
  ...
  const page11Result = await geminiModel.generateContent(
    `${presentationData.topic} mavzusining ${plan[8]} rejasi asosida 80 - 90 so'zlardan iborat 1 paragraf ma'lumot ber!`
  );
  ...
  elevenSlide.addText(plan[8].toUpperCase(), {
    x: 0.8,
    y: 0.6,
    align: "center",
    fontSize: 22,
  ...
  const page12Result = await geminiModel.generateContent(
    `${presentationData.topic} mavzusining ${plan[9]} rejasi asosida 80 - 90 so'zlardan iborat 1 paragraf ma'lumot ber!`
  );
  ...
  twelveSlide.addText(plan[9].toUpperCase(), {
    x: 0.8,
    y: 0.6,
    align: "center",
    fontSize: 22,
  ...
  const page13Result = await geminiModel.generateContent(
    `${presentationData.topic} mavzusida 80 - 90 so'zlardan iborat 1 paragraf mavzu bo'yicha xulosa yozib ber!`
  );
